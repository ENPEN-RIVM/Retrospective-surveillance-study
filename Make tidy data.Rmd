...
for (rr in 1:N_files){
    pb_sa[(Length_sample_table*(rr-1)+2):(rr*Length_sample_table),c(1,2)] = pb_sa[(Length_sample_table*(rr-1)+1),c(1,2)]} 
...
# This function will be run for every year (2018-2023) seperately
freq_table <- function(pb_data, yr, coln, L){
  ...
  # The country, lab and new column name (Sample) are specified on the first row for the lab of interest, in columns 1, 2 and 3
  table[,c(1,2,4)] = as.matrix(setNames(pb_data[-((L)*(1:N_files)-(L-1)),c(1,2,3)],NULL))
  ...
  for (virus_index in 1:length(Virs)){                                          
    ...
    Z  = which(pb_data==EV, arr.ind=TRUE)
    # If there is a hit at all
    if (length(Z)>0){         
      # Loop over the rows and columns listed in matching cells
      for (index in 1:nrow(Z)){                                
        # Save the row number in r
        r         = Z[index,1]    
        # Save the column number in c
        c         = Z[index,2]       
        # Find the lab of of the match
        Fil       = pb_data$File[r]   
        # For this specific match [r, c], the new table with designated virus (column) and lab, will be filled with the numbers that are listed underneath the virus type in the original dataset
        table[which(table$Lab==Fil),virus_index+4]=pb_data[r+(1:(L-1)),c]
        }
      }     
    }
